...
func gameOfLife(board [][]int) {
	...
	stepi := []int{-1, -1, -1, 0, 1, 1, 1, 0}
	stepj := []int{-1, 0, 1, 1, 1, 0, -1, -1}
	for i := 0; i < len(board); i++ {
		for j := 0; j < len(board[0]); j++ {
			if board[i][j] == 0 {
				t := 0
				for x := 0; x < len(stepi); x++ {
					if ni, nj := i+stepi[x], j+stepj[x]; ni >= 0 && ni < len(board) && nj >= 0 && nj < len(board[0]) && board[ni][nj] == 1 {
						t += 1
					}
				}
				if t == 3 {
